...
int dcli_wlan_method_parse(char *DBUS_METHOD){
	...
	else if(!strcmp(DBUS_METHOD,WID_DBUS_WLAN_METHOD_SHOW_QOS_MAP_INFO))
	{
		sn = 6;/* show qos-map-info */
	}	
	...
	switch(dcli_sn)
	{
		...
		case 2 :
		{
			...
			if((WLANINFO)&&(WLANINFO->WLAN)&&(WLANINFO->WLAN[0])){
				...
				CW_FREE_OBJECT(WLANINFO->WLAN[0]->eap_mac);
...
struct SSIDConfigInfo *show_SSIDConfig_info_of_all_wlan(int index,int localid,DBusConnection *dcli_dbus_connection, unsigned int *num, unsigned int *ret)
{
	...
	memset(NewSSIDName,0,ESSID_DEFAULT_LEN);
	...
	*num = wlan_num;	
	for(i = 0; i < wlan_num; i++)
	{
		...
		if(!(WlanNode->NewSSIDName=(char *)malloc(strlen(NewSSIDName)+1)))
		{
		...
		memset(WlanNode->NewSSIDName,0,strlen(NewSSIDName)+1);
		memcpy(WlanNode->NewSSIDName,NewSSIDName,strlen(NewSSIDName));
	...
	if (*ret == 0 )
	{
		...
		switch (dcli_sn)
		{
			case 1 :
			{
				...
				for (i = 0; i < LIST->wlan_num; i++)
				{
					...
					LIST->WLAN[i]->ESSID = (unsigned char*)malloc(ESSID_DEFAULT_LEN+1);
			...
			case 2 :
			{
				...
				LIST->WLAN[0]->ESSID= (unsigned char*)malloc(ESSID_DEFAULT_LEN+1);
				...
				dbus_message_iter_next(&iter);		
				dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->aclgrp_id));

				dbus_message_iter_next(&iter);		
				dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->WDSStat));
				
				dbus_message_iter_next(&iter);		
				dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->electronic_menu));
				
				dbus_message_iter_next(&iter);		
				dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->macgroupID));
				
				dbus_message_iter_next(&iter);		
				dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->mac_grp_policy));
		...
		case 5 :
		{
			...
			case 6 :
			{
				...
				for (i = 0; i < MAX_QOS_11E_VAL+1; i++)
				{
					dbus_message_iter_next(&iter);	
					dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ac_11e_to_1p[i]));				
				}
				for (i = 0; i < MAX_QOS_DSCP_VAL+1; i++)
				{
					dbus_message_iter_next(&iter);	
					dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ac_dscp_to_1p[i]));				
				}
				for(i = 0; i < MAX_QOS_DSCP_VAL+1; i++)
				{
					dbus_message_iter_next(&iter);	
					dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ac_dscp_to_dscp[i]));				
				}
				
				dbus_message_iter_next(&iter);	
				dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ap_11e_to_1p_switch));	
				
				for (i = 0; i < MAX_QOS_11E_VAL+1; i++)
				{
					dbus_message_iter_next(&iter);	
					dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ap_11e_to_1p[i]));				
				}
				
				dbus_message_iter_next(&iter);	
				dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ap_11e_to_dscp_switch));	
				
				for (i = 0; i < MAX_QOS_11E_VAL+1; i++)
				{
					dbus_message_iter_next(&iter);	
					dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ap_11e_to_dscp[i]));				
				}
				for (i = 0; i < MAX_QOS_DSCP_VAL+1; i++)
				{
					dbus_message_iter_next(&iter);	
					dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ap_dscp_to_11e[i]));				
				}
				for (i = 0; i < MAX_QOS_1P_VAL+1; i++)
				{
					dbus_message_iter_next(&iter);	
					dbus_message_iter_get_basic(&iter,&(LIST->WLAN[0]->qos_ap_1p_to_11e[i]));				
				}
			}
			break;
			case 7 :{}
...
unsigned int dcli_wlan_number_triger
(
	int index,
	int localid,
	unsigned char wlanid,
...
)
{
	...
	ReInitDbusPath_V2(localid,index,WID_DBUS_WLAN_OBJPATH,OBJPATH);
	ReInitDbusPath_V2(localid,index,WID_DBUS_WLAN_INTERFACE,INTERFACE);
	query = dbus_message_new_method_call(BUSNAME,OBJPATH,INTERFACE,WID_DBUS_WLAN_METHOD_APPAY_WLAN_BALANCE_TRIGER);	
