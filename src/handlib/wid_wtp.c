...
void dcli_free_allwtp_neighbor_ap(struct allwtp_neighborap *all_wtp_neighbor_apNODE)
{
	...
	CW_FREE_OBJECT(all_wtp_neighbor_apNODE);
...
void dcli_free_neighborapInfo_node(struct Neighbor_AP_ELE *neighborapInfos_node)
{
	...
	struct Neighbor_AP_ELE *t = neighborapInfos_node;
	...
	while(t !=NULL)
	{
		tmp =t;
		t = t->next;
		DCLI_FORMIB_FREE_OBJECT(tmp->IEs_INFO);
		tmp->next = NULL;
		CW_FREE_OBJECT(tmp);
	}
}
...
const char *wid_parse_inet_ntoa(unsigned int address, char *addr_p)
{
...
int check_ip_range(unsigned long ipaddr,unsigned long endip,char * WTPIP)
{
	unsigned long wtpip = wid_ip2ulong(WTPIP);

	if((endip >= ipaddr) && (wtpip >= ipaddr && wtpip <= endip))
	{
		return 1;
	}
	else if((endip < ipaddr) && (wtpip <= ipaddr && wtpip >= endip))
	{
		return 1;
	}
	else
...
{
	struct tag_wtpid *p = NULL;
	struct tag_wtpid *q = NULL;
	struct tag_wtpid *temp1 = NULL;
	struct tag_wtpid *temp2 = NULL;
	...
	if((*ret == 0)&&(*wtp_num != 0))
	{
		...
		for(i=0;i<(*wtp_num);i++)
		{
			...
			*ret = MALLOC_ERROR;
			...
			WtpNode->wtpMacAddr= (char *)malloc(MAC_LEN+1);
...
{	
	...
	else if((dcli_sn == 2)||(dcli_sn == 3)||(dcli_sn == 4)||(dcli_sn == 5)||(dcli_sn == 6)
			||(dcli_sn == 7)||(dcli_sn == 9)||(dcli_sn == 10)||(dcli_sn == 11)||(dcli_sn == 12))
	{
...
//printf("ccccccccc  ret is %d \n",ret);
	...
	//printf("dddddddddddddddddddddddddddddddddd \n");
			...
			dbus_message_iter_next(&iter);	
			dbus_message_iter_get_basic(&iter,&(LIST->WTP[0]->electronic_menu));
			dbus_message_iter_next(&iter);	
			dbus_message_iter_get_basic(&iter,&(LIST->WTP[0]->macgroupID));
			dbus_message_iter_next(&iter);	
			dbus_message_iter_get_basic(&iter,&(LIST->WTP[0]->mac_grp_policy));
			dbus_message_iter_next(&iter);	
			dbus_message_iter_get_basic(&iter,&(LIST->WTP[0]->comment));
	...
	/*		int j = 0;		
		...
		for (i = 0; i < LIST->num; i++) {
			...
			dbus_message_iter_get_basic(&iter_struct,&(LIST->AP_VERSION[i]->apmodel));	
			
			dbus_message_iter_next(&iter_struct);
		
			dbus_message_iter_get_basic(&iter_struct,&(LIST->AP_VERSION[i]->versionname));
		
			dbus_message_iter_next(&iter_struct);
			
			dbus_message_iter_get_basic(&iter_struct,&(LIST->AP_VERSION[i]->versionpath));
		...
		for (i = 0; i < LIST->num; i++) {
			...
			dbus_message_iter_get_basic(&iter_struct,&(LIST->WTP[i]->WTPIP));
		...
		*num = ret;
		...
		{	
			...
			{	
				...
				char i = 0, k = 0, m = 0;
				unsigned int j = 0;
				unsigned char Count_onetimeupdt = 0;
		...
		switch(dcli_sn){
			case 1 :{
				if((WTPINFO)&&(WTPINFO->WTP)&&(WTPINFO->WTP[0])){
					...
					//CW_FREE_OBJECT(WTPINFO->WTP[0]->comment);
			...
			dbus_message_iter_recurse(&iter_struct,&iter_sub_array);
			 ...
			 for(j=0; j<WtpNode->wtpBwlanNum; j++){  
				...
				memset(sub_wlan_node->wlan_essid, 0, ESSID_DEFAULT_LEN+1);
				 ...
				 dbus_message_iter_next(&iter_sub_struct);
				 dbus_message_iter_get_basic(&iter_sub_struct,&wlan_essid);		//wlan essid
				 memcpy(sub_wlan_node->wlan_essid, wlan_essid, strlen(wlan_essid));
                           ...
                           if((sub_wlan_node = (struct SSIDStatsInfo_sub_wlan*)malloc(sizeof(struct SSIDStatsInfo_sub_wlan))) == NULL){
					           ...
					           *ret = MALLOC_ERROR;
				                ...
				                //printf("######sub_wlan_node->wtpSSIDLoadBalance = %d\n",sub_wlan_node->wtpSSIDLoadBalance);
								 ...
								 char *Essid = NULL;
								 dbus_message_iter_next(&iter_sub_sub_struct);	
								 dbus_message_iter_get_basic(&iter_sub_sub_struct,&Essid);//string
								  //printf("zenmehuishia\n");
								  //puts(Essid);
	...
	if(*ret == 0){
		...
		for (i = 0; i < *num; i++) {
			...
			for(j=0;j<WtpNode->rogue_ap_num;j++){
				...
				dbus_message_iter_next(&iter_sub_struct);
				dbus_message_iter_get_basic(&iter_sub_struct,&(rogueAPEssid));			//char *
				dbus_message_iter_next(&iter_sub_struct);
...
conflict_wtp_list_free(struct conflict_wtp_info *conflict_wtp)
{
	struct ConflictWtp *tmp_node = NULL;
...
unsigned int dcli_wid_set_wlan_balance_rssi
(
	unsigned int index,
...
{	
	...
	query = dbus_message_new_method_call(BUSNAME,OBJPATH,INTERFACE,WID_DBUS_WTP_METHOD_SET_WLAN_BALANCE_RSSI);
