	...
	{	
		...
		//	memset(LIST->WTP[0]->WTPMAC,0,(MAC_LEN +1));
				...
				//	RADIO[i] = (WID_WTP_RADIO*)malloc(sizeof(WID_WTP_RADIO));
					...
					dbus_message_iter_get_basic(&iter_struct,&(LIST->RADIO[i]->bss_num));
		...
		//	LIST->RADIO[0]->BSS = (WID_BSS*)malloc((LIST->bss_num)*sizeof(WID_BSS));
			for (i = 0; i < LIST->bss_num_int; i++) {
				LIST->RADIO[0]->BSS[i] = (WID_BSS*)malloc(sizeof(WID_BSS));
				memset(LIST->RADIO[0]->BSS[i], 0, sizeof(WID_BSS));
				LIST->RADIO[0]->BSS[i]->BSSID = (unsigned char *)malloc(6);
				memset(LIST->RADIO[0]->BSS[i]->BSSID,0,6);
			...
			//	memset(BSS[i]->BSSID,0,6);
				...
				dbus_message_iter_get_basic(&iter_struct, &(alias_essid));
				memcpy(LIST->RADIO[0]->BSS[i]->aliasESSID, alias_essid, ESSID_LENGTH);
				dbus_message_iter_next(&iter_struct);
				...
				dbus_message_iter_get_basic(&iter_struct,&(LIST->RADIO[0]->BSS[i]->keyindex));
...
int dcli_radio_11n_set_mixed_puren_switch
(
    int localid, 
    int index,
...
struct WtpList * set_radio_channel_cmd_channel
(
	int localid,
	int index,
...
struct WtpList *set_radio_txpower_cmd_txpower
(
	int localid,
	int index,
	DBusConnection *dbus_connection,
...
struct WtpList * set_radio_txpowerof_cmd_txpoweroffset
(
	int localid,
	int index,
	DBusConnection *dbus_connection,
...
struct WtpList *set_radio_ratelist_cmd_set_support_ratelist
(
	int localid,
	int index,
	DBusConnection *dbus_connection,
...
)
{
	...
	//int modes = 0;
	//int nums = 0;
	int list1[RADIO_RATE_LIST_LEN] = {0};
	int list2[RADIO_RATE_LIST_LEN] = {0};
	char fail_reason = 0;
	struct RadioList *RadioNode = NULL;
	struct RadioList *radio_list_head = NULL;
...
struct WtpList *set_radio_max_rate_cmd_set_max_rate_v1
(
    int localid, 
    int index,
    DBusConnection *dbus_connection,
...
void dcli_sa_head_deinit(struct sa_head *head)
{
	...
	{
		if (head->node[i].info && head->node[i].channel_num)
		{
			for (j = 0; j < head->node[i].channel_num; j++)
			{
				if (head->node[i].info[j].info)
				{
					head->node[i].info[j].disturb_num = 0;
					free(head->node[i].info[j].info);
					head->node[i].info[j].info = NULL;
				}
			}
		}
		
		head->node[i].channel_num = 0;
		free(head->node[i].info);
		head->node[i].info = NULL;
	}
...
unsigned int wid_show_radiolist_sa
(
	unsigned int index,
	int localid,
...
)
{
	...
	unsigned int disturb_num = 0;
	unsigned int radioid = 0;
	struct sa_res *res = NULL;
	...
	if ((!sa || !radiolist || !dcli_dbus_connection))
	{
		return WID_DBUS_ERROR;
	}
	...
	query = dbus_message_new_method_call(BUSNAME, OBJPATH, INTERFACE,
								WID_DBUS_RADIO_METHOD_SHOW_SPECTRUM_ANALYSIS_LIST);
																												
	dbus_error_init(&err);			
	dbus_message_iter_init_append(query, &iter);

	dbus_message_iter_append_basic(&iter, DBUS_TYPE_UINT32, &(radiolist->count));	
	...
	size = sizeof(struct sa_res) * radio_num;
	sa->node = (struct sa_res *)malloc(size);
	if (!sa->node)
	{
		return WID_DBUS_ERROR;
	}
	memset(sa->node, 0, size);
	sa->radio_num = radio_num;
	...
	for (i = 0; i < radio_num; i++)
	{
		...
		dbus_message_iter_get_basic(&iter, &(channel_num));
		dbus_message_iter_next(&iter); 
		...
		{		
			dbus_message_iter_get_basic(&iter, &(disturb_num));
			dbus_message_iter_next(&iter); 
			...
			dbus_message_iter_get_basic(&iter, &(channel_info[j].channel));
			dbus_message_iter_next(&iter); 
			
			for (k = 0; k < disturb_num; k++)
			{
				dbus_message_iter_get_basic(&iter, &(disturb_info[k].cnt));
				dbus_message_iter_next(&iter); 
				
				dbus_message_iter_get_basic(&iter, &(disturb_info[k].rssi));
				dbus_message_iter_next(&iter);

				dbus_message_iter_get_basic(&iter, &(disturb_info[k].type));
				dbus_message_iter_next(&iter);
			}
		}
	}
...
)
{
	...
	query = dbus_message_new_method_call(BUSNAME, OBJPATH, INTERFACE,
								WID_DBUS_RADIO_METHOD_SET_SPECTRUM_ANALYSIS_LIST);
	...
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_UINT32, &scan_cycle);	
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_UINT32, &scan_time);	
	dbus_message_iter_append_basic (&iter, DBUS_TYPE_UINT32, &report_interval);	
...
unsigned int wid_set_radiolist_sa_enable
(
	unsigned int index,
	int localid,
	update_radio_list *radiolist, 
...
)
{
	...
	query = dbus_message_new_method_call(BUSNAME, OBJPATH, INTERFACE,
								WID_DBUS_RADIO_METHOD_SET_SA_ENABLE);
																												
	dbus_error_init(&err);			
	dbus_message_iter_init_append(query, &iter);

	dbus_message_iter_append_basic(&iter, DBUS_TYPE_BYTE, &policy);
