...
$.extend(SVGManager.prototype, {
	...
	_connectSVG: function(container, loadURL, onLoad, settings) {
		var id = this._nextId++;
		var svgHtml = '<' + (($.browser && $.browser.msie) ? 'embed' : 'object') + 
			' width="' + container.clientWidth +
			'" height="' + container.clientHeight + '" type="image/svg+xml" ' +
			(($.browser && $.browser.msie) ? 'src' : 'data') + '="blank' + (($.browser && $.browser.msie) ? id : '') + '.svg' + 
			(($.browser && $.browser.msie) ? '' : '?' + id) + '"' + (($.browser && $.browser.msie) ? '/>' :
...
$.extend(SVGRoot.prototype, {
	...
	load: function(url, addTo) {
		...
		var http = $.ajax({url: url, dataType: 'xml', success: function(data) {
			if ($.browser && $.browser.msie) { // Doesn't load properly!
