...
{
    unsigned char *afi_mac_name = NULL;  
    afc_config_s * afcconf = NULL;
    ...
    if(afcconf)
    {
        ...
        remove_node(&(afcconf->afi_blist.blist_conf),(unsigned char *)testMac);
    }
    save_config_info(afcconf);
...
EXT_FUNCTION(ext_get_system_config)
{
	zval *iter_len;
	...
	add_property_zval(return_value, "value", iter_len);
	add_property_long(return_value, "result", (long)((afcconf == NULL)? -1 : 0));
}
