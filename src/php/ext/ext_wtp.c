...
ext_func_handle_t wtp_func_list[] = {
    ...
    {"set_afi_map_config", 7, (php_func_t)ext_set_afi_map_config},
};
...
int set_afi_map_info_save_config
(
	unsigned int wtpid, 
	char* mapname, 
	unsigned int left, 
...
{/*when add new wtp or set wtp name call this function */
	...
	if(!(wtpid >= START_WTPID && wtpid <= MAX_WTP_ID))
	{
	    return -1;
	}
    ...
    {        
        ...
        {
        	syslog(LOG_ERR, FUNC_LINE_FORMAT" afi%d not found when set map info!", FUNC_LINE_VALUE, wtpid);
                return -1;
        }
        
        wtp_tmpnode = afcconf->wtps[wtpid];
        memset(wtp_tmpnode->mapname, 0, MID_STRING_LEN);
        wtp_tmpnode->mapleft = left;
		wtp_tmpnode->maptop = top;
		wtp_tmpnode->maplock = lock;
        
        strncpy(wtp_tmpnode->mapname, mapname, MID_STRING_LEN-1);
...
EXT_FUNCTION(ext_sta_list)
{
	...
	for (i = 0; i < STALIST.sta_num; i++)
	{
		...
		add_assoc_string(iter_array, "deviceType", (char *)tmpMacStr, 1);
...
//		syslog(LOG_DEBUG, "before call show_network_wtp_config wtpid %d", wtpid);
		...
		if (wtpwebinfo.isused)
		{
			...
			if(afcconf->wtps[wtpid])
			{
				struct wtp_conf * wtpnode = afcconf->wtps[wtpid];
				add_assoc_string(iter_array, "mapname", wtpnode->mapname, 1);
				add_assoc_long(iter_array, "mapleft", (long)wtpnode->mapleft);
				add_assoc_long(iter_array, "maptop", (long)wtpnode->maptop);
				add_assoc_long(iter_array, "maplock", (long)wtpnode->maplock);
			}
			...
			while (wtpwebinfo.wlans[j].WlanId)
			{
				...
				add_assoc_long(iter_array_r, "others_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[Others]);
				add_assoc_long(iter_array_r, "iphone_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[iPhone]);
				add_assoc_long(iter_array_r, "phone_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[Phone]);
				add_assoc_long(iter_array_r, "ipad_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[iPad]);
				add_assoc_long(iter_array_r, "pad_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[Pad]);
				add_assoc_long(iter_array_r, "macbook_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[MacBook]);
				add_assoc_long(iter_array_r, "pc_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[PC]);
				add_assoc_long(iter_array_r, "mi_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_type_num[MI]);
				add_assoc_long(iter_array_r, "wlan_sta", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_num);
				add_assoc_long(iter_array_r, "auth_per", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_auth_per);
				add_assoc_long(iter_array_r, "abn_deauth_per", (long)wtpwebinfo.wtp_wlan_wifi.wlan_wifi[j].sta_abn_deauth_per);
...
EXT_FUNCTION(ext_set_afi_map_config)
{
	int ret = 0;
	long wtpid = 0;
	char * mapname = NULL;
	...
	ext_para_get(argc, argv, EXT_TYPE_LONG, &wtpid, EXT_TYPE_STRING, &mapname,
		EXT_TYPE_LONG, &left, EXT_TYPE_LONG, &top, EXT_TYPE_LONG, &lock,
		EXT_TYPE_STRING, &admin);
	
	if(!(wtpid >= START_WLANID && wtpid <= MAX_WTP_ID ))
	{
		syslog(LOG_ERR, FUNC_LINE_FORMAT" get bad parameter wtpid %ld ",FUNC_LINE_VALUE, wtpid);
		RETURN_LONG(-1);
	}
	
	ret = set_afi_map_info_save_config((unsigned int)wtpid, (char *)mapname, (unsigned int)left, (unsigned int)top, (BOOL)lock);
	if(ret < 0)
	{
		syslog(LOG_WARNING, FUNC_LINE_FORMAT" save config for add new wtp failed!", FUNC_LINE_VALUE);
	}
