...
void get_afi_blacklist_conf(xmlNodePtr pcurnode, afc_config_s * afcconf)
{
    ...
    {	 
    	...
    	if ((!xmlStrcmp(testnode->name, BAD_CAST  CONF_BLOCKLIST))) 
    	{
    		// value = xmlNodeGetContent(testnode);
      //       DEBUG_VALUE(testnode);
    		//afcconf->afi_blist.wtp_auto_access = (unsigned char )(0 == strcmp((char *)value, "true"));
    		get_afi_block_list_conf(testnode,afcconf);
    	}
		else
		{
			syslog(LOG_WARNING, "unknow node to parse for conf file: %s", (char *)testnode->name);
...
void get_user_blacklist_conf(xmlNodePtr pcurnode, afc_config_s * afcconf)
{
    ...
    {	 
      ...
      //       DEBUG_VALUE(testnode);
    		//afcconf->afi_blist.wtp_auto_access = (unsigned char )(0 == strcmp((char *)value, "true"));
    		get_user_block_list_conf(testnode,afcconf);
    	}
		else
		{
			syslog(LOG_WARNING, "unknow node to parse for conf file: %s", (char *)testnode->name);
...
void destroy_afc_conf(afc_config_s * afcconf, int freeafc)
{
    if(afcconf)
    {
        ...
        destroy_block_list(&afcconf->user_blist.blist_conf);
        destroy_block_list(&afcconf->afi_blist.blist_conf);
        for(i = 0; i <= MAX_WLANID; i++)
        {
...
int read_afc_config_from_conf_file(char * pathFile, afc_config_s * afcconf)
{
	...
	{     
		...
		else if ((!xmlStrcmp(pcurnode->name, BAD_CAST CONF_AFI_BLACKLIST)))
		{
		    DEBUG_NAME(pcurnode);
			get_afi_blacklist_conf(pcurnode, afcconf);
		}
		else if ((!xmlStrcmp(pcurnode->name, BAD_CAST CONF_USER_BLACKLIST)))
		{
		    DEBUG_NAME(pcurnode);
			get_user_blacklist_conf(pcurnode, afcconf);
		}
...
void set_user_block_list_conf(xmlNodePtr pcurnode, afc_config_s * afcconf)
{
	...
	tmp_bnode = afcconf->user_blist.blist_conf.head;
    while(tmp_bnode)
	{
	    char tmpMacStr[32] = {0};
	    snprintf(tmpMacStr, 31, "%02x:%02x:%02x:%02x:%02x:%02x", 
...
void set_afi_block_list_conf(xmlNodePtr pcurnode, afc_config_s * afcconf)
{
	...
	tmp_bnode = afcconf->afi_blist.blist_conf.head;
    while(tmp_bnode)
	{
	    char tmpMacStr[32] = {0};
	    snprintf(tmpMacStr, 31, "%02x:%02x:%02x:%02x:%02x:%02x", 
...
void set_afi_blacklist_conf(xmlNodePtr pcurnode, afc_config_s * afcconf)
{
    ...
    while(tmp_node !=NULL)
	{
		if( (!xmlStrcmp(tmp_node->name, BAD_CAST CONF_BLOCKLIST)) )
		{
           //set_xml_node_value_from_bool(tmp_node, (BOOL)afcconf->wireless_global.wtp_auto_access);
			set_afi_block_list_conf(tmp_node,afcconf);
		}
		tmp_node = tmp_node->next;
	}
	return;
...
void set_user_blacklist_conf(xmlNodePtr pcurnode, afc_config_s * afcconf)
{
    ...
    while(tmp_node !=NULL)
	{
		...
		{
           //set_xml_node_value_from_bool(tmp_node, (BOOL)afcconf->wireless_global.wtp_auto_access);
			set_user_block_list_conf(tmp_node,afcconf);
		}
		tmp_node = tmp_node->next;
	}
	return;
...
int write_afc_config_to_conf_file(char * pathFile, afc_config_s * afcconf)
{
	...
	while(pcurnode != NULL) 
	{
		if (!xmlStrcmp(pcurnode->name, BAD_CAST CONF_SYSTEM))
	  	...
	  	{
            DEBUG_NAME(pcurnode);
	  		set_afi_blacklist_conf(pcurnode, afcconf);
		}
		else if (!xmlStrcmp(pcurnode->name, BAD_CAST CONF_USER_BLACKLIST))
	  	{
            DEBUG_NAME(pcurnode);
	  		set_user_blacklist_conf(pcurnode, afcconf);
		}
		else if (!xmlStrcmp(pcurnode->name, BAD_CAST CONF_WIRELESS_GLOBAL))
	  	{
            DEBUG_NAME(pcurnode);
