...
Global $procBarForm, $procBarLabel, $procBarProgress, $procBarButton, $proBarPic, $afcHostIp
Global $initPercent, $progressCounter, $debugLabel
;Global $procBarFormA, $procBarProgressA, $procBarButtonA, $proBarPicA
...
If FileExists (@ScriptDir&"\update.exe") Then
  Sleep (2000)
  msgbox($MB_SYSTEMMODAL,"DEBUG提示","删除update.exe",0)
...
If NOT FileExists ($var1) Then
  msgbox($MB_SYSTEMMODAL,"DEBUG提示","初始化settings.ini",0)
...
If IniRead ($var1, "lang", "key", "NotFound") = 0 Then
  ...
  Local $WS_POPUP
  ;msgbox($MB_SYSTEMMODAL,"DEBUG提示","读取语言设置",0)
...
;msgbox($MB_SYSTEMMODAL,"DEBUG提示","当前语言设置 " & $lng,0)
If IniRead ($var1, "update", "key", "NotFound") = 1 Then
  ;msgbox($MB_SYSTEMMODAL,"DEBUG提示","获取"&$updateUrl&"update.data",0)
...
If FileExists (@TempDir&"\update.ini") Then
  ...
  Endif
EndIf
;GUICtrlSetData($procBarProgress, 5)
$initPercent = 5

If $new1 = 1 OR $new2 = 1 Then
  Global $Input300, $Checkbox200
  Global $update = 1
  ;msgbox($MB_SYSTEMMODAL,"DEBUG提示","版本比较 "& $new1 & " starter " & $new2,0)
...
If IniRead (@ScriptDir&"\data\settings\settings.ini", "update", "key", "NotFound") = 1 Then
  ;msgbox($MB_SYSTEMMODAL,"DEBUG提示","检测update.ini",0)
  Sleep (2000)
  FileDelete (@TempDir&"\update.ini")
EndIf

;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","HybridMode",0)
; Thibaut : use Hybrid Mode if available
HybridMode()

;GUICtrlSetData($procBarProgress, 10)
$initPercent = 10
If NOT (FileExists (@ScriptDir&"\app32") OR FileExists (@ScriptDir&"\app64")) Then
  ...
  Local $WS_POPUP
  ;msgbox($MB_SYSTEMMODAL,"DEBUG提示","检测app",0)
...
Else
  Global $startvbox = 1
EndIf
;msgbox($MB_SYSTEMMODAL,"DEBUG提示","即将开始启动vbox "& $startvbox ,0)
;GUICtrlSetData($procBarProgress, 15)
$initPercent = 15
...
EndIf
 ...
 ;GUICtrlSetData($procBarProgress, 20)
 $initPercent = 20
  ...
  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","即将开始启动VBoxManage "& $arch ,0)   
  If NOT ProcessExists ("VirtualBox.exe") OR NOT ProcessExists ("VBoxManage.exe") Then
      ...
      If IniRead ($var1, "hotkeys", "key", "NotFound") = 1 Then
		...
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","带热键注册系统托盘",0)
      ...
      Else
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","注册系统托盘",0)
	  ...
	  ;GUICtrlSetData($procBarProgress, 25)
	  $initPercent = 25
      If @OSArch = "x86" Then
        If NOT FileExists (@SystemDir&"\msvcp71.dll") OR NOT FileExists (@SystemDir&"\msvcr71.dll") OR NOT FileExists (@SystemDir&"\msvcrt.dll") Then
		  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","x86系统复制msvc*.dll",0)
      ...
      If @OSArch = "x64" Then
        If NOT FileExists (@SystemDir&"\msvcp80.dll") OR NOT FileExists (@SystemDir&"\msvcr80.dll") Then
		  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","x64系统复制msvc*.dll",0)
      ...
      If FileExists (@ScriptDir&"\"& $arch &"\") AND FileExists (@ScriptDir&"\vboxadditions\") Then
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","复制VBox扩展",0)
	  ...
	  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","注册VBoxSVC",0)
	  If $debugMode = 1 Then
		GUICtrlSetData($debugLabel, "进度提示:1-1/9 "& $arch & "\VBoxSVC.exe /reregserver")
		sleep(2000)
	  EndIf
      RunWait ($arch&"\VBoxSVC.exe /reregserver", @ScriptDir, @SW_HIDE)
	  If $debugMode = 1 Then
		GUICtrlSetData($debugLabel, "进度提示:1-2/9 "& @SystemDir & "\regsvr32.exe /S " & $arch & "\VBoxC.dll")
		sleep(2000)
	  EndIf
      RunWait (@SystemDir&"\regsvr32.exe /S "& $arch &"\VBoxC.dll", @ScriptDir, @SW_HIDE)
	  If $debugMode = 1 Then
		GUICtrlSetData($debugLabel, "进度提示:1-3/9 DllCll('" & $arch & "\VBoxRT.dll', 'hwnd', 'RTR3Init')")
		sleep(2000)
	  EndIf
      DllCall ($arch&"\VBoxRT.dll", "hwnd", "RTR3Init")

      ;SplashOff ()
	  ;GUICtrlSetData($procBarProgress, 30)
	  $initPercent = 30
      ;If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxDRV", "DisplayName") <> "VirtualBox Service" Then
	  If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxDRV", "DisplayName") <> "PortableVBoxDRV" Then
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","注册VBoxDRV",0)
		If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:1-4/9 cmd /c sc create VBoxDRV binpath= '%CD%\" & $arch & "\drivers\VBoxDrv\VBoxDrv.sys' type= kernel start= auto error= normal displayname= PortableVBoxDRV")
			sleep(2000)
		EndIf
	  ...
	  If IniRead ($var1, "usb", "key", "NotFound") = 1 Then
        If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxUSB", "DisplayName") <> "VirtualBox USB" Then
          If @OSArch = "x86" Then
			MsgBox($MB_SYSTEMMODAL,"DEBUG提示","x86系统配置VBoxUSB",0)
          ...
          EndIf
          If @OSArch = "x64" Then
			MsgBox($MB_SYSTEMMODAL,"DEBUG提示","x64系统配置VBoxUSB",0)
      ...
      If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxUSBMon", "DisplayName") <> "VirtualBox USB Monitor Driver" Then
        MsgBox($MB_SYSTEMMODAL,"DEBUG提示","注册VBoxUSBMon",0)
      ...
      #cs If IniRead ($var1, "net", "key", "NotFound") = 1 Then
        If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxNetAdp", "DisplayName") <> "VirtualBox Host-Only Network Adapter" Then
          MsgBox($MB_SYSTEMMODAL,"DEBUG提示","注册VBoxNetAdp",0)
		  If @OSArch = "x86" Then
	  ...
	  #ce
	  Local $ADP = 0
	  ;GUICtrlSetData($procBarProgress, 35)
	  $initPercent = 35
	  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","注册VBoxSVC结束",0)
      If IniRead ($var1, "net", "key", "NotFound") = 1 Then
        If RegRead ("HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VBoxNetFlt", "DisplayName") <> "VirtualBox Bridged Networking Service" Then
          ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","注册VBoxNetFlt",0)
		  If @OSArch = "x86" Then
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:1-5/9 " & @ScriptDir & "\data\tools\snetcfg_x86.exe -v -u sun_VBoxNetFlt")
				sleep(2000)
			EndIf
            RunWait (@ScriptDir&"\data\tools\snetcfg_x86.exe -v -u sun_VBoxNetFlt", @ScriptDir, @SW_HIDE)
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:1-6/9 " & @ScriptDir & "\data\tools\snetcfg_x86.exe -v -l .\" & $arch &"\drivers\network\netflt\VBoxNetFlt.inf -m .\"& $arch &"\drivers\network\netflt\VBoxNetFltM.inf -c s -i sun_VBoxNetFlt")
				sleep(2000)
			EndIf
          ...
          If @OSArch = "x64" Then
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:1-5/9 " & @ScriptDir & "\data\tools\snetcfg_x64.exe -v -u sun_VBoxNetFlt")
				sleep(2000)
			EndIf
            RunWait (@ScriptDir&"\data\tools\snetcfg_x64.exe -v -u sun_VBoxNetFlt", @ScriptDir, @SW_HIDE)
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:1-6/9 " & @ScriptDir & "\data\tools\snetcfg_x64.exe -v -l .\"& $arch &"\drivers\network\netflt\VBoxNetFlt.inf -m .\"& $arch &"\drivers\network\netflt\VBoxNetFltM.inf -c s -i sun_VBoxNetFlt")
				sleep(2000)
			EndIf
		  ...
		  If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:1-7/9 " & @SystemDir & "\regsvr32.exe /S "& @SystemDir &"\VBoxNetFltNobj.dll")
			sleep(2000)
		  EndIf
      ...
      If $DRV = 1 Then
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动VBoxDRV",0)
		If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:1-8/9 sc start VBoxDRV")
			sleep(2000)
		EndIf
        RunWait ("sc start VBoxDRV", @ScriptDir, @SW_HIDE)
      EndIf
	  ;GUICtrlSetData($procBarProgress, 40)
	  $initPercent = 40

	  #cs
      If $USB = 1 Then
		MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动VBoxUSB",0)
	  ...
	  #cs
      If $MON = 1 Then
		MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动VBoxUSBMon",0)
	  ...
	  #cs
      If $ADP = 1 Then
		MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动VoxNetAdp",0)
        RunWait ("sc start VBoxNetAdp", @ScriptDir, @SW_HIDE)
      EndIf
	  #ce

      If $NET = 1 Then
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动VBoxNetFlt",0)
		If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:1-9/9 sc start VBoxNetFlt")
			sleep(2000)
		EndIf
        RunWait ("sc start VBoxNetFlt", @ScriptDir, @SW_HIDE)
      EndIf
	  ;GUICtrlSetData($procBarProgress, 45)
	  $initPercent = 45
	  ; Create and register existing VM
	  Global $DefaultVm = IniRead($var1, "afc", "vm", "AFC")
	  Local $DefaultDisk = IniRead($var1, "afc", "disk", "%CD%\vdisk\afc.vdi")
	  local $RegVm = IniRead($var1, "afc", "createvm" , 0)
	  local $headless = IniRead($var1, "afc", "header", "")
	  Local $nadapter = IniRead($var1, "afc", "adapter", "")
	  
	  If $RegVm = 0 then 
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","创建VM " & $DefaultVm, 0)
		  If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:2-1/4 cmd /c .\" & $arch & "\VBoxManage.exe createvm -register -name " & $DefaultVm)
			sleep(2000)
		  EndIf
		  RunWait("cmd /c .\" & $arch & "\VBoxManage.exe createvm -register -name " & $DefaultVm, @ScriptDir, @SW_HIDE)
		  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","指定VM " & $DefaultVm & " storage", 0)
		  ;GUICtrlSetData($procBarProgress, 50)
		  $initPercent = 50
		  If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:2-2/4 cmd /c .\" & $arch & "\VBoxManage.exe storagectl """ & $DefaultVm &""" --name ""SATA0"" --add sata --controller IntelAHCI --portcount 1 --bootable on")
			sleep(2000)
		  EndIf
		  RunWait("cmd /c .\" & $arch & "\VBoxManage.exe storagectl """ & $DefaultVm &""" --name ""SATA0"" --add sata --controller IntelAHCI --portcount 1 --bootable on", @ScriptDir, @SW_HIDE)
		  ;GUICtrlSetData($procBarProgress, 55)
		  $initPercent = 55
		  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","指定VM " & $DefaultVm & " storageattach" & $DefaultDisk, 0)
		  If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:2-3/4 cmd /c .\" & $arch & "\VBoxManage.exe storageattach """ & $DefaultVm & """ --storagectl ""SATA0"" --port 0 --device  0 --type hdd --medium """& $DefaultDisk & """")
			sleep(2000)
		  EndIf
		  RunWait("cmd /c .\" & $arch & "\VBoxManage.exe storageattach """ & $DefaultVm & """ --storagectl ""SATA0"" --port 0 --device  0 --type hdd --medium """& $DefaultDisk & """", @ScriptDir, @SW_HIDE)
		  ;GUICtrlSetData($procBarProgress, 60)
		  $initPercent = 60
		  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","指定VM " & $DefaultVm & " bridge adapter", 0)
		  If $nadapter = "" Then
			$nadapter = getipinfo()
			If $nadapter <> "" Then
				IniWrite($var1, "afc", "adapter", $nadapter)
			EndIf
		  EndIf
		  If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:2-4/4 cmd /c .\" & $arch & "\VBoxManage.exe modifyvm """ & $DefaultVm & """ --ostype linux --nic1 bridged --bridgeadapter1 """& $nadapter & """ --macaddress1 auto --memory 512")
			sleep(2000)
		  EndIf
		  RunWait("cmd /c .\" & $arch & "\VBoxManage.exe modifyvm """ & $DefaultVm & """ --ostype linux --nic1 bridged --bridgeadapter1 """& $nadapter &""" --macaddress1 auto --memory 512", @ScriptDir, @SW_HIDE)
		  ;GUICtrlSetData($procBarProgress, 65)
		  $initPercent = 65
		  IniWrite($var1, "afc", "createvm", 1)
	  Endif
	  	
	  Global $afc_running = 0

      If $CmdLine[0] = 1 Then
	    ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","即将启动VirtualBox.exe cmdLine[0] " & $CmdLine[0] & @ScriptDir,0)
        If FileExists (@ScriptDir&"\data\.VirtualBox") Then
          ...
          If IniRead ($var1, "userhome", "key", "NotFound") = "%CD%\data\.VirtualBox" AND FileExists (@ScriptDir&"\data\.VirtualBox\HardDisks\"&$CmdLine[1]&".vdi") Then
			;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动带参设置VBOX_USER_HOME="& $UserHome & "\" & $arch & "\VBoxManage.exe startvm " & $StartVM & "",0)
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:3-1/1 cmd /c set VBOX_USER_HOME="& $UserHome &"& .\"& $arch &"\VBoxManage.exe startvm """& $StartVM &"")
				sleep(2000)
			EndIf
			RunWait ("cmd /c set VBOX_USER_HOME="& $UserHome &"& .\"& $arch &"\VBoxManage.exe startvm """& $StartVM &"" , @ScriptDir, @SW_HIDE)
			;GUICtrlSetData($procBarProgress, 70)
			$initPercent = 70
          Else
			;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动带参设置VBOX_USER_HOME="&$UserHome & "\" & $arch & "\VBoxManage.exe",0)
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:3-1/1 cmd /c set VBOX_USER_HOME="& $UserHome &"& .\"& $arch &"\VirtualBox.exe")
				sleep(2000)
			EndIf
          ...
          EndIf
        Else
		  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动带参设置VBOX_USER_HOME=%CD%\data\.VirtualBox & .\" & $arch & "\VBoxManage.exe startvm" & $StartVM & "",0)
		  If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:3-1/1 cmd /c set VBOX_USER_HOME=%CD%\data\.VirtualBox & .\"& $arch &"\VirtualBox.exe")
			sleep(2000)
		  EndIf
		...
		;GUICtrlSetData($procBarProgress, 75)
		$initPercent = 75
      Else
		Local $fe = FileExists(@ScriptDir&"\data\.VirtualBox")
		;MsgBox($MB_SYSTEMMODAL, "DEBUG提示", "check " & @ScriptDir & "\data\.VirtualBox found " & $fe, 0) 
        If FileExists (@ScriptDir&"\data\.VirtualBox") <> 0 Then
          Local $UserHome = IniRead ($var1, "userhome", "key", "NotFound")
          Local $StartVM  = IniRead ($var1, "startvm", "key", "NotFound")

          ;If IniRead ($var1, "startvm", "key", "NotFound") = true Then
		  If $StartVM <> "NotFound" Then
			;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动无参设置0VBOX_USER_HOME="&$UserHome & "&.\" & $arch & "\VBoxManage.exe startvm " & $StartVM & "",0)
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:3-1/1 cmd /C set VBOX_USER_HOME="& $UserHome &"& .\"& $arch &"\VBoxManage.exe startvm "& $StartVM &" "&$headless)
				sleep(2000)
				If $headless <> "" Then
					$headless = ""
				EndIf
			EndIf
            RunWait ("cmd /C set VBOX_USER_HOME="& $UserHome &"& .\"& $arch &"\VBoxManage.exe startvm "& $StartVM &" "&$headless , @ScriptDir, @SW_HIDE)
          Else
			;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动无参设置1VBOX_USER_HOME="&$UserHome & "&.\" & $arch & "\VBoxManage.exe",0)
			If $debugMode = 1 Then
				GUICtrlSetData($debugLabel, "进度提示:3-1/1 cmd /c set VBOX_USER_HOME="& $UserHome &"& .\"& $arch &"\VirtualBox.exe")
				sleep(2000)
			EndIf
		  ...
		  $initPercent = 70
        Else
		  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动无参设置2VBOX_USER_HOME=%CD%\data\.VirtualBox & .\" & $arch & "\VBoxManage.exe startvm """ & $StartVM & "",0)
		  If $debugMode = 1 Then
			GUICtrlSetData($debugLabel, "进度提示:3-1/1 cmd /c set VBOX_USER_HOME=%CD%\data\.VirtualBox & .\"& $arch &"\VirtualBox.exe")
			sleep(2000)
          EndIf
		...
		while $afc_running <> 1
			;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","启动AFC进度检查",0)
			sleep(2000)
			_CheckAfcProcess()
			If $initPercent <= 85 Then
				$initPercent += 2
			EndIf
		WEnd
		...
		;Repaint process bar
		#cs
		GUICtrlDelete($proBarPic)
		$proBarPic = GUICtrlCreatePic(@ScriptDir&"\data\settings\AFC-Bar-Min.jpg",-1, -1, 720,260)
		GUISetState(@SW_SHOW)
		#ce
		Local $waitAfcVM = 14
		While $waitAfcVM > 0 
	  ...
	  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","ProcessWaitClose VBoxSVC",0)
      ...
      If $msv = 2 Then
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","删除msvc*.dll",0)
      ...
      SplashOff ()
    Else
	  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","重置Oracle VM VirtualBox Manager服务",0)
      ;WinSetState ("Oracle VM VirtualBox Manager", "", BitAND (@SW_SHOW, @SW_RESTORE))
	  ;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","重置] - Oracle VM VirtualBox服务",0)
...
Func ExitScript ()
	...
	If $StartVM <> "NotFound" Then
		RunWait ("cmd /C set VBOX_USER_HOME="& $UserHome &"& .\"& $arch &"\VBoxManage.exe controlvm """& $StartVM &""" poweroff" , @ScriptDir, @SW_HIDE)
	EndIf
	ProcessWaitClose ("VirtualBox.exe")
	ProcessWaitClose ("VBoxManage.exe")
	ProcessWaitClose ("VBoxHeadless.exe")
...
Func HybridMode()
	...
	;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","HybridMode arch " & @OSArch & "VER %VER% version " & $version_new, 0)
	...
	;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","HybridMode arch " & @OSArch & "version old " & $version_new, 0)
	...
	If $version_new <> "" AND StringLeft($version_new,1)>=4 Then
		...
		;MsgBox($MB_SYSTEMMODAL,"DEBUG提示","HybridMode arch " & @OSArch & "cmd " & $CmdLine[0] & " " & $CmdLine[1],0)
		if $CmdLine[0] = 1 Then
...
; Check if AFC start successfully
Func _CheckAfcProcess()
	Local $GuestIpFile = "guestip.txt"
	Local $arch = "app32"
	;MsgBox($MB_SYSTEMMODAL, "DEBUG提示", "get guest property "& $DefaultVm & " failed", 0)
	sleep(1000)
	...
	If $FileHdl = -1 then
		;MsgBox($MB_SYSTEMMODAL, "DEBUG提示", "open "& $GuestIpFile & " failed", 0)
		;GUICtrlSetData($procBarProgress, 80)
		Return False
	Else
		Local $FileCtent = FileRead($FileHdl)
		;MsgBox($MB_SYSTEMMODAL, "DEBUG提示", "content " & $FileCtent, 0)
		
		If StringInStr($FileCtent, "Value: ") <> 0 then
			Local $nParams = StringSplit($FileCtent, ": ", $STR_ENTIRESPLIT)
			;MsgBox($MB_SYSTEMMODAL, "DEBUG提示", "split " & $nParams[0] & "..." & $nParams[2] , 0)
			;AdlibUnRegister()
			;GUICtrlSetData($procBarProgress, 85)
			$afcHostIp = $nParams[2]
			sleep(10000)
			FileClose($FileHdl)
			...
			$afc_running = 1
		Else 
			If StringInStr($FileCtent, "No value set") = 0 then
				;GUICtrlSetData($procBarProgress, 85)
				sleep(1000)
				$afc_running = 1
			Endif
...
Func InitProcessBar()
	...
	$procBarLabel = GUICtrlCreateLabel("正在努力加载AFC... ...", 50, 29, 220, 36)
	...
	$procBarButton = GUICtrlCreateButton("启动AFC", 268, 179, 185, 41)
	GUICtrlSetBkColor($procBarButton, 0x0070C0)
	GUICtrlSetColor($procBarButton, 0xFFFFFF)
	_GUICtrlButton_Enable($procBarButton, False)
	
EndFunc

; Initialize process bar window
#cs
Func RepaintProcessBar()
	GUIDelete($procBarForm)
	sleep(500)
	Local $procBarFormA = GUICreate("启动AFC", 720, 260, -1, -1, BitOR($WS_DLGFRAME,$WS_BORDER), $WS_EX_APPWINDOW)
	GUISetIcon(@ScriptDir&"\data\settings\AFC-VM-min.ico", -1)
	GUISetCursor (2)
	GUISetFont(16, 400, 0, "华文细黑")
	GUISetBkColor(0x00B0F0)
	...
	Local $proBarPicA = GUICtrlCreatePic(@ScriptDir&"\data\settings\AFC-Bar-Max.jpg", -1, -1, 720,260)
	GUISetState(@SW_SHOW)
	 
	Global $procBarProgressA = GUICtrlCreateProgress(50, 88, 601, 41, 0x01)
	GUICtrlSetBkColor($procBarProgressA, 0xFFFFFF)
	GUICtrlSetColor(-1, 0xFFCF35)
	GUICtrlSetData($procBarProgressA, 100)
	GUISetState(@SW_SHOW)
	
	Local $procBarButtonA = GUICtrlCreateButton("登录AFC", 268, 179, 185, 41)
	GUICtrlSetBkColor($procBarButtonA, 0x0070C0)
	GUICtrlSetColor($procBarButtonA, 0xFFFFFF)
	GUICtrlSetOnEvent ($procBarButtonA, "StartAFCWeb")
	_GUICtrlButton_Enable($procBarButtonA, True)
	GUISetState(@SW_SHOW)
EndFunc
#ce
...
Func RefreshProgress()
	#cs
	If $initPercent <= 30 Then
		$progressCounter += 5
	Else 
		If $initPercent <=70 Then
			$progressCounter += 4
		Else 
			If $initPercent <= 85 Then
				$progressCounter += 3
			Else 
				$progressCounter = $initPercent
			EndIf
		EndIf
	EndIf
	
	If $initPercent = 100 And $progressCounter <= 98 Then
		$progressCounter += 2
	EndIf
	#ce
